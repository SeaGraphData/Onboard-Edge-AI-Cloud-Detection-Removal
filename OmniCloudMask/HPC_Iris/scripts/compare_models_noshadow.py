...
GEOAI_NOSHADOW   = Path("/scratch/users/jfernandezmartinez/GeoAI/results/masks_binary_noshadow")
...
OUT_DIR = Path("/scratch/users/jfernandezmartinez/GeoAI/results/comparison_noshadow")
...
def make_error_map(pred, gt):
    ...
    rgb[(pred == 1) & (gt == 1)] = [0.0, 0.8, 0.0]
    rgb[(pred == 0) & (gt == 0)] = [0.8, 0.9, 1.0]
    rgb[(pred == 1) & (gt == 0)] = [0.9, 0.1, 0.1]
    rgb[(pred == 0) & (gt == 1)] = [1.0, 0.6, 0.0]
    return rgb

def make_diff_map(pred_cd, pred_ocm, gt):
    h, w = gt.shape
    rgb = np.zeros((h, w, 3), dtype=np.float32)
    rgb[(pred_cd == 1) & (pred_ocm == 1) & (gt == 1)] = [0.0, 0.8, 0.0]
    rgb[(pred_cd == 0) & (pred_ocm == 0) & (gt == 0)] = [0.85, 0.9, 0.95]
    rgb[(pred_cd == 1) & (pred_ocm == 0) & (gt == 1)] = [0.2, 0.4, 0.9]
    rgb[(pred_cd == 0) & (pred_ocm == 1) & (gt == 1)] = [1.0, 0.6, 0.0]
    rgb[(pred_cd != gt) & (pred_ocm != gt)]            = [0.9, 0.1, 0.1]
    return rgb
...
for fold in FOLDS:
    ...
    for patch_dir in tqdm(patches, desc=fold):
        ...
        ocm_ns_path     = GEOAI_NOSHADOW / (patch_name + ".png")

        if not ocm_ns_path.exists():
            print(f"  [WARN] OmniCloud noshadow mask not found: {patch_name}")
        ...
        ocm_ns     = load_binary_png(ocm_ns_path)
        ...
        ocm_error_map = make_error_map(ocm_ns,  cd_gt)
        diff_map      = make_diff_map(cd_pred, ocm_ns, cd_gt)
        ocm_overlay_arr = make_ocm_overlay(true_color, ocm_4class)

        cd_metrics  = compute_metrics(cd_pred, cd_gt)
        ocm_metrics = compute_metrics(ocm_ns,  cd_gt)
        agreement   = pixel_agreement(cd_pred, ocm_ns)

        # ── Figure 3 rows × 5 cols ─────────────────────────────────────────────
        fig, axes = plt.subplots(3, 5, figsize=(25, 16))

        # Row 1 — CD-Mamba
        axes[0,0].imshow(cd_rgb)
        axes[0,0].set_title("CD-Mamba: RGB input", fontsize=9)
        axes[0,1].imshow(cd_gt, cmap='gray', vmin=0, vmax=1)
        axes[0,1].set_title("GT mask (binary)", fontsize=9)
        axes[0,2].imshow(cd_pred, cmap='gray', vmin=0, vmax=1)
        axes[0,2].set_title("CD-Mamba prediction", fontsize=9)
        axes[0,3].imshow(cd_overlay)
        axes[0,3].set_title("CD-Mamba overlay", fontsize=9)
        axes[0,4].imshow(cd_error_map)
        axes[0,4].legend(handles=error_legend, loc='lower right', fontsize=6)
        axes[0,4].set_title("CD-Mamba error map", fontsize=9)

        # Row 2 — OmniCloudMask (no shadow)
        axes[1,0].imshow(true_color)
        axes[1,0].set_title("True Color (R/G/B)", fontsize=9)
        axes[1,1].imshow(cd_gt, cmap='gray', vmin=0, vmax=1)
        axes[1,1].set_title("GT mask (binary)", fontsize=9)
        axes[1,2].imshow(ocm_ns, cmap='gray', vmin=0, vmax=1)
        axes[1,2].set_title("OmniCloud binarized\n(thick+thin only, no shadow)", fontsize=9)
        ...
        axes[1,4].imshow(true_color)
        axes[1,4].imshow(ocm_overlay_arr)
        axes[1,4].set_title("OmniCloud overlay", fontsize=9)

        # Row 3 — Differences
        axes[2,0].imshow(diff_map)
        axes[2,0].legend(handles=diff_legend, loc='lower right', fontsize=6)
        axes[2,0].set_title("Difference map\n(CD-Mamba vs OmniCloud no shadow)", fontsize=9)
        axes[2,1].imshow(false_color)
        axes[2,1].set_title("False Color (NIR/R/G)", fontsize=9)
        axes[2,2].imshow(ocm_error_map)
        axes[2,2].legend(handles=error_legend, loc='lower right', fontsize=6)
        axes[2,2].set_title("OmniCloud error map (no shadow)", fontsize=9)
        ...
        axes[2,3].set_title("Metrics comparison (no shadow)", fontsize=9)

        # Bar chart
        metric_names = ['F1','Precision','Recall','IoU','Kappa','CSI','Dice']
        cd_vals  = [cd_metrics[k]  for k in ['f1','precision','recall','iou','kappa','csi','dice']]
        ocm_vals = [ocm_metrics[k] for k in ['f1','precision','recall','iou','kappa','csi','dice']]
        x = np.arange(len(metric_names))
        w = 0.35
        axes[2,4].bar(x - w/2, cd_vals,  w, label='CD-Mamba',  color='steelblue',  alpha=0.85)
        axes[2,4].bar(x + w/2, ocm_vals, w, label='OmniCloud', color='darkorange', alpha=0.85)
        for i, (cv, ov) in enumerate(zip(cd_vals, ocm_vals)):
            axes[2,4].text(i - w/2, cv + 0.01, f'{cv:.2f}', ha='center', va='bottom', fontsize=6)
            axes[2,4].text(i + w/2, ov + 0.01, f'{ov:.2f}', ha='center', va='bottom', fontsize=6)
        axes[2,4].set_xticks(x)
        axes[2,4].set_xticklabels(metric_names, fontsize=7, rotation=30)
        axes[2,4].set_ylim(0, 1.15)
        axes[2,4].set_ylabel("Score (0–1)", fontsize=8)
        axes[2,4].legend(fontsize=7)
        axes[2,4].set_title("Metrics bar chart", fontsize=9)
        axes[2,4].grid(axis='y', alpha=0.3)
        ...
        fig.suptitle(
            f"{fold} — {patch_name}  |  Pixel agreement: {agreement:.4f}  "
            f"[binarisation: thick+thin cloud only, shadow excluded]",
            fontsize=9, y=1.01
        )
        plt.tight_layout()
        plt.savefig(OUT_DIR / fold / (patch_name + "_comparison_noshadow.png"),
        ...
        all_results.append({
            ...
            "cdmamba":         cd_metrics,
            "omnicloud_noshadow": ocm_metrics,
        })
...
if all_results:
    ...
    ocm_means = {k: np.mean([r['omnicloud_noshadow'][k]   for r in all_results]) for k in metric_keys}
    ...
    axes[0].bar(x + w/2, [ocm_means[k] for k in metric_keys], w,
                label='OmniCloud (no shadow)', color='darkorange', alpha=0.85)
    axes[0].set_xticks(x)
    axes[0].set_xticklabels([k.replace('_','\n') for k in metric_keys], fontsize=8)
    axes[0].set_ylim(0, 1.05)
    axes[0].set_ylabel("Score (0–1)", fontsize=9)
    axes[0].legend(fontsize=9)
    axes[0].set_title(
        f"Global mean metrics — 60 patches\n"
        f"Pixel agreement: {mean_agr:.4f} | binarisation: thick+thin cloud only",
        fontsize=10
    )
    axes[0].grid(axis='y', alpha=0.3)

    cd_f1s  = [r['cdmamba']['f1']             for r in all_results]
    ocm_f1s = [r['omnicloud_noshadow']['f1']   for r in all_results]
    ...
    axes[1].set_ylabel("OmniCloud F1 (no shadow)", fontsize=10)
    axes[1].set_xlim(0,1); axes[1].set_ylim(0,1)
    axes[1].set_title("F1 per patch: CD-Mamba vs OmniCloud (no shadow)", fontsize=10)
    axes[1].legend(fontsize=9)
    axes[1].grid(alpha=0.3)

    plt.tight_layout()
    plt.savefig(OUT_DIR / "summary_comparison_noshadow.png", dpi=120, bbox_inches='tight')
    plt.close(fig)
    print(f"\n[INFO] Summary figure saved.")

# ── JSON ───────────────────────────────────────────────────────────────────────
out_json = OUT_DIR / "comparison_noshadow_results.json"
with open(out_json, "w") as f:
    json.dump({
        "binarisation":    "thick+thin cloud only (shadow class excluded)",
        ...
        "global_means": {
            ...
            "omnicloud_noshadow": {k: round(float(v),4) for k,v in ocm_means.items()},
        },
