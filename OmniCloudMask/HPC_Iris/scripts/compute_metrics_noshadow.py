...
BINARY_DIR = Path("/scratch/users/jfernandezmartinez/GeoAI/results/masks_binary_noshadow")
...
print(f"[INFO] Binary masks (no shadow) found: {len(binary_files)}")
...
for pred_path in tqdm(binary_files, desc="Computing metrics (no shadow)"):
...
results = {
    "model":        "OmniCloudMask v1.7.1 (no cloud shadow)",
    "dataset":      "Biome (Landsat-8)",
    "binarisation": "thick cloud + thin cloud only (class 1 and 2, shadow excluded)",
...
out_json = OUT_DIR / "results_omnicloudmask_noshadow.json"
with open(out_json, "w") as f:
    json.dump(results, f, indent=2)

out_txt = OUT_DIR / "results_omnicloudmask_noshadow.txt"
with open(out_txt, "w") as f:
    f.write("=" * 60 + "\n")
    f.write("OmniCloudMask — Biome Evaluation (no cloud shadow)\n")
    f.write("=" * 60 + "\n\n")
    f.write(f"Binarisation   : thick + thin cloud only (shadow excluded)\n")
    f.write(f"Patches        : {n}\n\n")
    ...
    f.write("Global Metrics (pixel-level, shadow excluded):\n")
