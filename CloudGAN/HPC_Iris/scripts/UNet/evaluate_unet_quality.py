...
# -*- coding: utf-8 -*-
"""
evaluate_unet_quality.py — Cloud Detection Quality Metrics for U-Net
"""
...
WEIGHTS    = os.path.join(PROJECT, "weights", "unet_checkpoint.h5")
...
OUTPUT_DIR = os.path.join(PROJECT, "evaluation", "results", "unet_quality_images")
RESULT_JSON = os.path.join(PROJECT, "evaluation", "results", "unet_quality.json")
RESULT_TXT  = os.path.join(PROJECT, "evaluation", "results", "unet_quality.txt")
...
print("=" * 60)
print("  U-NET — Cloud Detection Quality Evaluation")
print("=" * 60)

print("\n[1/4] Loading UNet model...")
assert os.path.exists(WEIGHTS), "UNet weights not found: {}".format(WEIGHTS)
model = Unet(256, "relu", "sigmoid")
...
for i, img_path in enumerate(img_files):
    ...
    if num_saved < NUM_SAMPLES_TO_SAVE:
        ...
        cv2.imwrite(os.path.join(OUTPUT_DIR, "unet_{:04d}_{}.png".format(num_saved, os.path.splitext(basename)[0])), composite)
        num_saved += 1
...
results = {
    "model": "U-Net", "task": "Cloud Detection",
