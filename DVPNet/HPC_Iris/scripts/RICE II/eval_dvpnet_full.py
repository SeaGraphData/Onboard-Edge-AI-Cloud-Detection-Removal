"""
DVPNet Full Evaluation Script
Computes: Parameters, Size, FLOPs, Latency, FPS, GPU Memory,
          Power, Energy, PSNR, SSIM, MAE, MSE, RMSE, BRMSE, SAM
...
# ── Argument parsing ────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description='DVPNet Full Evaluation')
...
parser.add_argument('--input_dir',     required=True,  help='Cloudy images folder')
parser.add_argument('--input_truth_dir', required=True, help='Ground truth folder')
parser.add_argument('--result_dir',    default='output/rice1-eval', help='Output images folder')
...
model_size_mb = total_params * 4 / (1024 ** 2)   # float32 → bytes → MB
...
results = {
    "model":            "DVPNet",
    "dataset":          os.path.basename(os.path.dirname(args.input_dir.rstrip('/'))),
...
sep = "="*60
print(f"\n{sep}")
print(f"  DVPNet FULL EVALUATION — {results['dataset'].upper()}")
print(sep)
...
json_path = os.path.join(args.result_dir, f"dvpnet_eval_{now_str}.json")
txt_path  = os.path.join(args.result_dir, f"dvpnet_eval_{now_str}.txt")

with open(json_path, 'w') as f:
    json.dump(results, f, indent=4)

with open(txt_path, 'w') as f:
    f.write(f"DVPNet FULL EVALUATION — {results['dataset'].upper()}\n")
